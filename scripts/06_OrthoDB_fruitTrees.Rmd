...
process_OrthoDB_OrthoFinder <- function(inDir, fn, to_plant, refGenome, to_sub_pattern) {
  ...
  ## OrthoDB
  # Filter your df.subset by to_plant outside this function; assume passed in environment
  df.subset = df[df$speciesAbbr == to_plant, ]
  
  # Replace : with _ in gene_IDs for consistent matching
  ...
  # Rename columns and add metadata columns
...
data.table::fwrite(combo, '../intermediate/OrthoDB_fruitdiv.txt', sep = '\t')
zip(zipfile = "../intermediate/OrthoDB_fruitdiv.txt.zip", files = "../intermediate/OrthoDB_fruitdiv.txt")
file.remove("../intermediate/OrthoDB_fruitdiv.txt")
