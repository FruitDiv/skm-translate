...
_Denote_: disregarding scoring system
...
process_RBH <- function(input_dir, file_name, from_plant, to_plant, from_sub_pattern, to_sub_pattern) {
  ...
  colnames = c("query_id", "subject_id")
  df_sub = df[, ..colnames]
  ...
  if (!missing(to_sub_pattern) && !is.null(to_sub_pattern)) {
    df_sub$to = sub(to_sub_pattern, '', df_sub$to)
      if (to_plant == 'stu') {
        ...
        df_sub[DMT2DMG, on = .(to = v4_PGSC_DMT), to := geneID] 
      }     
  }
...
params_list = list(
  list(file_name = 'ath_stu.txt', to_plant = 'stu', to_sub_pattern = "(\\.[0-9]+)+$"), # .1 and .1.1 + exception
  list(file_name = 'ath_sly.txt', to_plant = 'sly', to_sub_pattern = "\\.[^.]*$"),
  list(file_name = 'ath_vvi.txt', to_plant = 'vvi', to_sub_pattern = "(_[^_]*){2}$"), # remove everything second from the last '_'
  
  list(file_name = 'ath_mdo.txt', to_plant = 'mdo', to_sub_pattern = "\\.[^.]*$"), # everything after the last dot
  list(file_name = 'ath_pper.txt', to_plant = 'ppe', to_sub_pattern = "\\.[^.]*$"), # everything after the last dot
  list(file_name = 'ath_pdul.txt', to_plant = 'pdul', to_sub_pattern = "\\.[^.]*$"), # everything after the last dot
  ...
  list(file_name = 'ath_pcox.txt', to_plant = 'pcox', to_sub_pattern = "\\.[^.]*$"), # everything after the last dot
...
file_path = '../intermediate/RBH_cleaned.txt'
